...
class Trainer():
    def __init__(self,Model,GARMMA,lr,batch_size=1000,update_target_per=100,global_step=0):
        ...
        self.GARMMA = GARMMA
        self.Model = Model
        for param in self.Model.Eval_net.parameters():
            param.requires_grad = False
        self.optimizer = torch.optim.Adam(self.Model.Target_net.parameters(), lr=lr)
    ...
    def get_loss(self, data, return_err=False):
        ...
        with torch.no_grad():
            y = (r*(1-self.GARMMA) + self.GARMMA *
                    self.Model(in_=torch.cat([a, s_t_plus_1], dim=-1), name="Eval")).detach()

        input_x = torch.autograd.Variable(torch.cat([a, s], dim=-1), requires_grad=True)
        
        R_, r_, backcast = self.Model(in_=input_x, name="Target", return_Q=False)
        ...
        tmp = -100*((s_t_plus_1[:, :len(self.Model.s_dim)]-s[:, :len(self.Model.s_dim)])*(R_))  # cache
        ...
        loss_backcast = torch.mean(torch.square(backcast - s[..., :len(self.Model.s_dim)]))
    ...
    def __call__(self, Memory):
        ...
        for data in data_loader:
            ...
            if self.global_step % self.update_target_per == 0:
                self.Model.Eval_net.load_state_dict(self.Model.Target_net.state_dict())
